...
class PostDesktopExtractor(PostExtractor):
    ...
    def extract_post_reactions(self):
        ...
        json_path = parse("$..cannot_see_top_custom_reactions.top_reactions")
        matches = json_path.find(self.post_data)
        if matches:
    ...
    def extract_post_comment(self):
        logger.info("Start")
        comment = 0
        json_path = parse("$..top_level_comment_list_renderer.feedback.comment_count")
        matches = json_path.find(self.post_data)
        if matches:
            comment_data = matches[0].value
